...
AStarPlanner::findPathBoundaries(const std::vector<grid_map::Position>& path, double search_radius) {
    ...
    // 处理第一个点
    if (path.size() > 1) {
        auto boundaries = findBoundaryForPoint(path[0], path[0], path[1], search_radius);
    ...
    for (size_t i = 1; i < path.size() - 1; ++i) {
        auto boundaries = findBoundaryForPoint(path[i], path[i-1], path[i+1], search_radius);
        left_boundary.push_back(boundaries[0]);
        right_boundary.push_back(boundaries[1]);
    }
    ...
    // 处理最后一个点
    if (path.size() > 1) {
        auto boundaries = findBoundaryForPoint(path.back(), path[path.size()-2], path.back(), search_radius);
        left_boundary.push_back(boundaries[0]);
        right_boundary.push_back(boundaries[1]);
    }
...
std::vector<grid_map::Position> 
AStarPlanner::findBoundaryForPoint(const grid_map::Position& point, 
                                  const grid_map::Position& prev_point,
